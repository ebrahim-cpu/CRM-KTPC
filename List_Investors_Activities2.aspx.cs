...
namespace CRM
{
    public partial class List_Investors_Activities2 : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                Session["FromLocation"] = "List_Investors_Activities2";
                BindCompanyDropdown();

                // Check for company ID in query string for pre-selection
                if (!string.IsNullOrEmpty(Request.QueryString["id"]))
                {
                    string companyName = GetCompanyNameById(Convert.ToInt32(Request.QueryString["id"]));
                    if (!string.IsNullOrEmpty(companyName) && ddlCompanyName.Items.FindByValue(companyName) != null)
                    {
                        ddlCompanyName.SelectedValue = companyName;
                    }
                }
                else
                {
                    ddlCompanyName.SelectedValue = ""; // Default to "All"
                }
        ...
        private void BindCompanyDropdown()
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("SELECT DISTINCT Company_Name FROM Investors_Profile ORDER BY Company_Name", con);
        ...
        protected void ddlCompanyName_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["CRMConnectionString"].ConnectionString))
            {
                ...
                cmd.Parameters.AddWithValue("@Company_Name", ddlCompanyName.SelectedValue); // Masukkan value dari ddl ke dlm @Company_Name
        ...
        protected void ddlInvestorStatus_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {                
                ...
                string q5 = "ORDER BY Activity_Date_Start DESC";
                string sqlStr = q1 + q2 + q3 + q4 + q5;
                SqlCommand arahan = new SqlCommand(sqlStr, con);
                arahan.Parameters.AddWithValue("@Company_Name", ddlCompanyName.SelectedValue);
                arahan.Parameters.AddWithValue("@Company_Status", ddlInvestorStatus.SelectedValue);
                // Execute the command and retrieve the results
                DataTable dt = new DataTable();
                dt.Load(arahan.ExecuteReader());
                // Bind the results to the Gridview
                gvActivities.DataSource = dt;
                gvActivities.DataBind();
                // Close the connection
        ...
        protected void displayAll()
        {
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["CRMConnectionString"].ConnectionString))
            {
                ...
                string q5 = "ORDER BY Activity_Date_Start DESC";
                string sqlStr = q1 + q2 + q3 + q4 + q5;
                SqlCommand arahan = new SqlCommand(sqlStr, con);                // Execute the command and retrieve the results
                DataTable dt = new DataTable();
                dt.Load(arahan.ExecuteReader());
                // Bind the results to the Gridview
                gvActivities.DataSource = dt;
        ...
        protected void gvActivities_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            {                
                con.Open();                
                SqlCommand arahan = new SqlCommand("DELETE FROM Investors_Activities WHERE Id = @Id", con);
                arahan.Parameters.AddWithValue("@Id", selectedId);                
        ...
        protected void btnFiles_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect("FileManage_Investor_Activities2.aspx?id=" + id);
        }
        ...
        protected void gvActivities_RowEditing(object sender, GridViewEditEventArgs e)
        {
            ...
            Response.Redirect("Edit_Investors_Activities2.aspx?ID=" + id);
        }
